...
private:
    ...
    {
        ROS_INFO("Takeoff requested! at controller");
        m_state = TakingOff;
        ...
        case TakingOff:
            {
                tf::StampedTransform transform;
                m_listener.lookupTransform(m_worldFrame, m_frame, ros::Time(0), transform);
                if (transform.getOrigin().z() > m_startZ + 0.05 || m_thrust > 50000)  // Record: thrust
                {
                    pidReset();
        ...
        case Automatic:
            {
                ...
                std::cout << ""+m_worldFrame+" _and_: "+ m_frame << "\n";
                ...
                */
                m_listener.transformPose(m_frame, targetWorld, targetDrone);        // Record: all parameters here.
                ...
                tf::Matrix3x3(
                    ...
                    )).getRPY(roll, pitch, yaw);                                    // Record: roll, pitch, yaw (NOTE: i am recording yaw in cmd_vel)


                // publish cmd_ver controller's output.
                geometry_msgs::Twist msg;
...
private:
    ...
    PID m_pidX;         // make sure how to know which pid is what
    PID m_pidY;
    PID m_pidZ;
    PID m_pidYaw;
    State m_state;
